...
3. В среду исполнения установить следующие пакеты: random_user_agent, webdriver-manager, BeautifulSoup4, Selenium, Pillow, lxml.
```
...
"logs-cleanup": true
```
При включении будут сохраняться только те логи, которые относятся к командам `mlp parce` и `mlp update`.
